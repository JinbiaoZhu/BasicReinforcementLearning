"""
Run AC.
"""

import os

import gym
import numpy as np
from tqdm import tqdm

from AC_algo import AC
from AC_config import ac_config
from evaluation import SimpleEvaluate
from tools import set_seed, print_dict

if __name__ == '__main__':

    print_dict(os.path.basename(__file__), ac_config)

    env = gym.make(ac_config["env_name"])

    num_episodes = ac_config["num_episode"]
    num_iteration = ac_config["num_iteration"]

    state_dim = env.observation_space.shape[0]
    action_dim = env.action_space.shape[0]
    max_action = env.action_space.high[0]

    set_seed(env, ac_config["seed"])

    agent = AC(state_dim, action_dim,
               hidden_dim=ac_config["hidden_dim"],
               max_action=max_action,
               activate="relu",
               actor_lr=ac_config["actor_lr"],
               critic_lr=ac_config["critic_lr"],
               gamma=ac_config["gamma"],
               batch_size=ac_config["batch_size"],
               maxlen=ac_config["maxlen"],
               device=ac_config["device"],
               )

    evaluation = SimpleEvaluate(ac_config["result_dir"],
                                algo_name=ac_config["algo"], env_name=ac_config["env_name"],
                                requires_loss=True, save_results=True)

    for i in range(num_iteration):
        with tqdm(total=int(num_episodes / num_iteration), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes / num_iteration)):

                evaluation.episode_return_is_zero()

                state = env.reset()
                done = False

                while not done:
                    action, dist = agent.action(state)
                    next_state, reward, done, _ = env.step(action)
                    agent.store(state, action, reward, next_state, done, dist)
                    state = next_state
                    env.render()
                    evaluation.add_single_step_reward(reward)

                evaluation.episode_return_record()

                lv = agent.update()
                evaluation.add_single_update_loss(lv)

                if (i_episode + 1) % num_iteration == 0:
                    pbar.set_postfix({
                        'episode':
                            '%d' % (num_episodes / num_iteration * i + i_episode + 1),
                        'return':
                            '%.3f' % np.mean(evaluation.return_list[-10:])
                    })
                pbar.update(1)

    evaluation.plot_performance()
