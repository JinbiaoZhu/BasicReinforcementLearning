...
from A2C_algo import A2C
from A2C_config import a2c_config
from evaluation import SimpleEvaluate
from tools import set_seed, print_dict, render_interval

if __name__ == '__main__':

    print_dict(os.path.basename(__file__), a2c_config)

    env = gym.make(a2c_config["env_name"])

    num_episodes = a2c_config["num_episode"]
    num_iteration = a2c_config["num_iteration"]

    state_dim = env.observation_space.shape[0]
    action_dim = env.action_space.shape[0]
    max_action = env.action_space.high[0]

    set_seed(env, a2c_config["seed"])

    agent = A2C(state_dim, action_dim,
                hidden_dim=a2c_config["hidden_dim"],
                max_action=max_action,
                activate="relu",
                actor_lr=a2c_config["actor_lr"],
                critic_lr=a2c_config["critic_lr"],
                gamma=a2c_config["gamma"],
                batch_size=a2c_config["batch_size"],
                maxlen=a2c_config["maxlen"],
                device=a2c_config["device"],
                )

    evaluation = SimpleEvaluate(a2c_config["result_dir"],
                                algo_name=a2c_config["algo"], env_name=a2c_config["env_name"],
        ...
        with tqdm(total=int(num_episodes / num_iteration), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes / num_iteration)):
                ...
                while not done:
                    ...
                    state = next_state
                    render_interval(i_episode, 0, env)
