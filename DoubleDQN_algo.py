...
class DoubleDQN:
    def __init__(self,
        ...
        print("DoubleDQN: state_dim={}, action_dim={}, hidden_dim={}, lr={}, gamma={}, epsilon={}, target_f={}".format(
    ...
    def update(self):
        ...
        ## Calculate Q_{w-}(s',a')
        ### This is DQN's next_q_values!
        ### next_q_values = self.target_DiscreteQNet(next_states).max(1)[0].view((-1, 1))\
        next_actions = self.DiscreteQNet(states).max(1)[1].view(-1, 1)
        next_q_values = self.target_DiscreteQNet(next_states).gather(1, next_actions)
